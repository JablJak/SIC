...
class PatchReconstruction(nn.Module):
    ...
    def forward(self, x, img_size: tuple[int, int]):
        x = x.transpose(1, 2)
        x = torch.unflatten(x, 2, img_size)
        x = self.conv_trans(x)
        return x
