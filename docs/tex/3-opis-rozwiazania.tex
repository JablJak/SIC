...
Opisywany model bazuje na opracowanych przez ostatnią dekadę badań rozwiązaniach, które zostały opisane w poprzednim rozdziale. Wykorzystano autoenkoder wariacyjny z koderem entropijnym wspomaganym przez model kontekstowy oraz model hiperprior'a, ze względu na potwierdzoną skuteczność tego rozwiązania w eksperymentach. Praca koncentruje się na ocenie efektywności podejścia z użyciem transformerów, stąd zarówno główny koder jak i główny dekoder są sieciami transformerowymi. Są one w istocie, dostosowanymi na potrzeby rozwiązania, wariantami sieci Swin Transformer. Z kolei model hiperprior'a i model kontekstowy używają z warstw splotowych.
W przypadku tego pierwszego warstwy splotowe dokonują analizy głównej przestrzeni cech ukrytych, w celu wyodrębnienia z niej informacji dodatkowej (zmienne hiperukryte). Model entropijny hiperprior'a opisuje rozkład informacji pomocniczej w sposób nieparametryczny, zakładając niezależność między kanałami przestrzeni cech hiperukrytych. W praktyce model nie uczy jawnie gęstości prawdopodobieństwa a dystrybuanty, kształtowane przez pięciowarstwowy perceptron. Prawdopodobieństwa wyznaczane ze wspomnianych dystrybuant służą koderowi arytmetycznemu hiperprior'a do zapisania informacji pobocznej w strumieniu bitów. Symetryczny dekoder hiperprior'a, w oparciu o te same dystrybuanty, odkodowuje informację poboczną, która po transformacji hiperdekodera definiuje parametry rozkładu przestrzeni głównych cech ukrytych. Model entropijny odwzorowuje koncepcję zaprezentowaną na konferencji CVPR w 2021 roku\cite{he2021checkerboardcontextmodelefficient}. Reprezentacja ukryta jest kwantowana przez zaokrąglanie do najbliższej liczby całkowitej i, przy wsparciu modelu entropijnego, jest kodowana arytmetycznie do strumienia binarnego. Zakodowany ciąg bajtów zawiera całą informację skompresowanego obrazu, gotową do odkodowania w symetrycznym dekoderze.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{Model parametrów rozkładów:} Predykcje z modelu hiperprior'a i modelu kontekstowego są łączone poprzez konkatenację, co prowadzi do uzyskania tensora o wymiarze \( \left(B, 1536, \nicefrac{H}{16}, \nicefrac{W}{16} \right) \). Przekazuje się go do modelu entropijnego złożonego z trzech warstw splotowych o jądrze \( 1 \times 1 \), co efektywnie czyni je warstwami liniowymi. Ich wymiary przestrzenne pozostają niezmienne, tj. \( \left( \nicefrac{H}{16}, \nicefrac{W}{16} \right) \), zaś liczba kanałów skaluje się kolejno z 4N do \( \frac{10N}{3} \), z \( \frac{10N}{3} \) do \( \frac{8N}{3} \) i z \( \frac{8N}{3} \) do 2N.
		\item \
...
\begin{itemize}
	...
	\item wartości hiperparametrów modelu takie jak rozmiar wektora cech, liczba etapów, liczba głowic mechanizmu uwagi, rozmiar okna przesuwnego;
...
\subsection{Porównanie rozwiązania z podobnymi pracami}
Mając na uwadze rosnące zainteresowanie kompresją obrazów z użyciem sieci neuronowych oraz fakt, że od zaprezentowania transformera wizyjnego z oknem przesuwnym minęło niemal pięć lat, należy spodziewać się, iż nie jest to jedyna próba zbadania jego zastosowania w tym zadaniu. Warto w tym podrozdziale wspomnieć dwie prace i porównać je z niniejszą: badanie autorów Ming Lu i in. (2021)\cite{lu2021transformerbasedimagecompression} oraz Priyanki Mudgal (2025) \cite{mudgal2025variablerateimagecompression}.

Pierwsza z wymienionych prac wykazuje wiele zbieżności z opisywanym projektem. Jej autorom przyświecał ten sam cel stworzenia efektywnego kodeka w oparciu o sieci neuronowe oferującego kilka modeli (dokładnie osiem) oferujących różne stopnie kompresji, otrzymanych dzięki sterowaniu wartością parametru \(\lambda\). Architektura ta również wykorzystuje koncepcję autoenkodera wariacyjnego z koderem arytmetycznym, wspieranego przez modelowanie hiperprior'a i kontekstu elementów w sposób autoregresyjny. Wynika z tego, że w szerokim ujęciu prace są konceptualnie bardzo zbieżne. Można dostrzec także wiele podobieństw na poziomie szczegółów:
\begin{itemize}
	\item Wykorzystany zbiór danych --- w obu przypadkach skorzystano ze zbioru obrazów Flickr, choć w różnych wariantach --- chiński zespół użył wariantu Flickr 2W.
	\item Strategia optymalizacji --- w pracy referowanej zastosowano optymalizator Adam, natomiast w niniejszym opracowaniu pokrewny AdamW, z takim samym współczynnikiem uczenia wynoszącym \(10^{-4}\). Proces uczenia prowadzono przez 400 epok z wsadem ośmiu obrazów, co również zbiega się z parametrami procesu uczenia opisanymi w następnym rozdziale. Jako miarę jakości percepcyjnej w funkcji straty w obu przypadkach użyto błędu średniokwadratowego (MSE).
	\item W obu podejściach na wejściu sieci stosowano losowo wycinane fragmenty obrazów, przy czym w pracy Lu miały one rozmiar \(256 \times 256\).
	\item Zarówno w jednej, jak i w drugiej pracy wykorzystano transformację GDN.
	\end{itemize}

Mimo wielu podobieństw, należy wskazać najistotniejsze różnice. Pierwsza i kluczowa rozbieżność dotyczy samej architektury: proponowana przez zespół Lu zarówno w głównej ramie koder-dekoder jak i w module hiperprior'a stosuje naprzemiennie bloki transformera z warstwy splotowe. W opisywanej pracy przyjęto założenie, że bloki transformera mogą skutecznie zastąpić warstwy splotowe, dlatego ich użycie zostało zminimalizowane w głównym koderze i dekoderze do minimum. W module hiperprior'a twórcy pozostali przy zmniejszonej, lecz analogicznej do głównego autoenkodera konfiguracji warstw. Badacze użyli mechanizmu atencji także w modelowaniu kontekstu. W tej pracy przyjęto założenie, że modele parametrów rozkładów mają być lekkie i korzystać ze znanych rozwiązań, dlatego składają się z warstw splotowych. Warto również zwrócić uwagę na rezygnację autorów z dzielenia obrazu wejściowego na siatkę łatek i ze strategii skalowania w obrębie bloków pikseli. Za te operacje odpowiadają warstwy splotowe o kroku równym dwóm pikselom. Dla lepszego zobrazowania różnic na rysunku \ref{fig:tic} przedstawiono schemat architektury wspomnianego rozwiązania.

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{tic}
	\caption[Architektura sieci TIC]{Architektura sieci TIC zespołu Ming Lu i in.\cite{lu2021transformerbasedimagecompression}}
	\label{fig:tic}
\end{figure}

Druga ze wspomnianych prac wykazuje mniej podobieństw z tym opracowaniem. Jako cel autorka obrała przygotowanie jednego modelu, zdolnego kompresować obrazy w możliwie najbardziej efektywny sposób, o zadanym stopniu kompresji i z możliwością zdefiniowania regionów zainteresowania. Ponadto autorka postuluje, że wykorzystanie mechanizmu okien przesuwnych ogranicza pole recepcyjne sieci, co ma negatywnie przekładać się na modelowanie zależności długodystansowych. Aby osiągnąć wskazane cele i rozwiązać problem ograniczonego kontekstu badaczka wprowadziła kilka rozwiązań nieobecnych w niniejszej pracy.

Aby możliwe było dynamiczne sterowanie poziomem kompresji sieć, oprócz wejścia w postaci obrazu, musi także otrzymać informację o tym, jaki ma do dyspozycji budżet bitowy do zakodowania obrazu. Osiągnięto to poprzez matrycę współczynników z zakresu \(\left[0,1\right]\) dyktującą stopień kompresji, gdzie 0 oznacza najniższą jakość a 1 najwyższą. Warto zauważyć, że matryca ma wymiary takie same jak obraz wejściowy, co daje możliwość dostosowania różnych poziomów dla różnych regionów obrazu. Aby sygnał informacji o stopniu kompresji nie został utracony matryca ta jest dostarczana na wejście bloków transformera na każdej głębokości kodera. Wymagane jest jednak skalowanie i projekcja cech w celu dopasowania wymiaru do mapy cech, co dokonuje się z użyciem warstw splotowych. Komplementarną do macierzy współczynników kompresji stanowi maska ROI, wyznaczająca obszar zwiększonej precyzji rekonstrukcji. Obie te informacje wpływają na funkcję straty --- zniekształcenie jest ważone przez maskę ROI a przepływność bitowa przez matrycę współczynników kompresji. Reasumując, strategia optymalizacji kompromisu R-D w przytoczonej pracy została zrealizowana w sposób bardziej złożony i kreatywny niż w niniejszej pracy.

Kwestię krótkiego zasięgu mechanizmu atencji, ograniczonego przez okna analizy, rozwiązano stosując tzw. kontekst N-gramów. Pojedynczym -gramem czyli unigramem w pracy Mudgal jest pojedyncze okno lokalnej atencji, zaś N-gram to z kolei zbiór sąsiadujących okien w obszarze \(N \times N\). Jako optymalne przyjęto w pracy kontekst \(N=2\) okien, czyli wykorzystanie \(2 \times 2\) okien dla obliczenia kontekstu w jakim znajduje się pojedyncze okno. Unigramy poddaje się osadzeniu do wektora cech, 
w celu skompresowania informacji dla przyspieszenia obliczeń i poddaje splotowej wariacji mechanizmu atencji. Wynikiem operacji jest informacja o tym, w jakim sąsiedztwie znajduje się każde z okien, dodawana następnie do każdego elementu w oknie lokalnej atencji. Dzięki temu już na etapie pierwszej analizy bloku transformera, jeszcze zanim nastąpi przesunięcie okna, mechanizm atencji zostaje wzbogacony o informację z sąsiadujących okien.